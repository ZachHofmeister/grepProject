...
void commands(void) {
	...
	for (;;) {
		...
		if (addr1==0)
			addr1 = addr2;
		switch(c) { //TODO is this where g and p are? if so, can we remove all but g and p?
			...
			case 'e': setnoaddr(); if (vflag && fchange) { fchange = 0;	error(Q); } filename(c); init(); addr2 = zero; goto caseread;
			...
			case 'k': nonzero(); if ((c = getchr()) < 'a' || c > 'z') { error(Q); } newline(); names[c-'a'] = *addr2 & ~01; anymarks |= 01; continue;
			...
			case 't': move(1); continue;
			case 'u': nonzero(); newline(); if ((*addr2&~01) != subnewa) { error(Q); } *addr2 = subolda; dot = addr2; continue;
			...
			case 'w':
				...
				filename(c);
				if((!wrapp || ((io = open(file,1)) == -1) || ((lseek(io, 0L, 2)) == -1)) && (io = creat(file, 0666)) < 0) error(file);
				wrapp = 0;
				if (dol > zero) putfile(); exfile();
...
void add(int i) {
	if (i && (given || dol>zero)) { addr1--; addr2--; }
	squeeze(0);
	newline();
	append(gettty, addr2);
}
...
void global(int k) {
	...
	*gp++ = 0;
	...
	//Special case: g/.../d (avoid n^2 algorithm) vvv
...
void join(void) {
	...
	for (a1=addr1; a1<=addr2; a1++) {
		lp = getline(*a1);
		while (*gp = *lp++) if (gp++ >= &genbuf[LBSIZE-2]) error(Q);
	}
	lp = linebuf;
	gp = genbuf;
	while (*lp++ = *gp++) {}
		...
		if (execute(a1)){
			...
			do {
				...
				if (--m <= 0) {
					...
					if (!gsubf) break;
					//null RE match vvv
					if (span==0) { if (*loc2=='\0') break; loc2++; }
				}
...
char * place(char *sp, char *l1, char *l2) {
	while (l1 < l2) { *sp++ = *l1++; if (sp >= &genbuf[LBSIZE]) { error(Q); } }
	return(sp);
}
...
void move(int cflag) {
	...
	nonzero();
	if ((adt = address())==0) error(Q); // address() guarantees addr is in range
	newline();
	if (cflag) {
		unsigned int *ozero;
		int delta;
